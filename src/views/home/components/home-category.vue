<template>
  <div class='home-category' @mouseleave="categoryId=null">
    <ul class="menu">
      <li v-for="item in menuList" :key="item.id" :class="{active:categoryId&&categoryId===item.id}" @mouseenter="categoryId=item.id" >
    ...
    <!-- 弹层 -->
    <div class="layer">
      <h4>{{ categoryId !=='brand'?'分类':'品牌'}}推荐 <small>根据您的购买或浏览记录推荐</small></h4>
      <!-- 商品 -->
...
export default {
  ...
  setup () {
    ...
    // 获取品牌数据，尽量不使用async到setup上
    // const data=await findBrand()不能在setup上添加saync，setup默认不支持async
    findBrand().then(data => {
      console.log(data)
