...
export default {
  ...
  setup () {
    ...
    //   Message({ type: 'success', text: '测试' })
    })

    // 组合api中如何调用实例---选项api中直接使用this
    ...
    // 1.发送验证码
    // 1.1绑定发送验证码按钮事件
    // 1.2校验手机号，如果成功才去发送短信（定义API），请求成功开启60s的倒计时，不能再次点击，倒计时结束恢复
    ...
    // qq登录
    // 回调页面三种情况:1.注册过，已绑定，跳首页
    // 2.注册过，未绑定--进行手机号绑定-跳首页
    // 3.未注册--进行信息完善--跳转
    // 1. 准备span有id = qqLoginBtn----不能当前页面打开，所以直接改成链接的形式
